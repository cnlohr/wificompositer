...
void * MillPosSender( void * c )
{
   const char * sendline = "!getxyz\n";
...
void * MillPosReader( void * v )
{
   ...
   servaddr.sin_port=htons(5252);

   connect(sockfd, (struct sockaddr *)&servaddr, sizeof(servaddr));


	FILE * f = fopen( "milllog.txt", "a" );

	OGCreateThread( MillPosSender, 0 );

   while(1)
   {
      n=recvfrom(sockfd,recvline,10000,0,NULL,NULL);
      recvline[n]=0;
      printf("%f %s", LastRX, recvline );
      fprintf(f,"%f %s", LastRX, recvline );
...
int main( int argc, char ** argv )
{
	...
	OGCreateThread( MillPosReader, 0 );
